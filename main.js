    ...
    function showAlert() {
      ...
      }, 20); // timeout kecil supaya trigger transition
    }
    ...
    function closeAlert() {
      ...
      // tunggu animasi selesai (300ms) sebelum sembunyikan sepenuhnya
      setTimeout(() => {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // **Langsung ubah tombol loading kembali ke submit**
          submitBtn.disabled = false;
          submitBtn.innerHTML = "Konfirmasi";

          // Kosongkan form langsung
          form.reset();
          form.querySelector('input[name="status"][value="hadir"]').checked = true;
...
// Smooth scroll manual untuk nav-link
document.querySelectorAll('a[href^="#"]').forEach(anchor => {
  anchor.addEventListener("click", function (e) {
    ...
    if (target) {
      window.scrollTo({
        top: target.offsetTop - 50, // offset biar ga ketutup nav
        behavior: "smooth"
      });
    }
  });
...
const observer = new IntersectionObserver(
  ...
  { 
    threshold: 0.5, 
    rootMargin: "-20% 0px -20% 0px" // lebih fleksibel: deteksi lebih awal
  }
);
